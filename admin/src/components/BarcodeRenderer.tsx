...
export const Code128Canvas: React.FC<Code128CanvasProps> = ({
  ...
  useEffect(() => {
    ...
    if (showText) {
      ctx.fillStyle = lightBackground ? '#111827' : '#f1f5f9';
    ...
    <div className={`inline-flex flex-col items-center select-none ${className}`}>
      ...
      {allowDownload && (
        <button
          ...
          className="mt-1.5 text-[10px] text-rose-400 hover:text-rose-300 underline flex items-center gap-1 cursor-pointer"
        >
...
export const BarcodeSVG: React.FC<BarcodeProps> = ({
  ...
  const textColor = lightBackground ? '#111827' : '#f1f5f9';
